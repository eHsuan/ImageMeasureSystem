...
{
    public partial class frmLCIN : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            //呼叫下一張影像
            if (dataGridViewMeasureData.RowCount >= 2 || dataGridViewMeasureData.Visible == false)
            {
                if (imagePathList.Count != 0)
                {
                    if (imagePathList.Count == imageNum + 1)
                    {
        ...
        public void PictureBoxBuild()
        {
            ...
            labelImageNum.Text = imageName.Split('_')[3].Split('-')[0];
            labelImageCount.Text = "1 / " + imagePathList.Count().ToString();

            if (imageName.Split('_')[3].Split('-')[1].Substring(0, 3).Equals("nor", StringComparison.OrdinalIgnoreCase))
            {
                imageType = ImageType.Normal;
                labelMeasureType.Text = "Normal";
                //確認是否有AI資料
        ...
        public void ChangeImage(string UpOrDown)
        {
            if (UpOrDown == "D")
            {
                if (imagePathList.Count != 0)
                {
                    ...
                    else
                    {
                        ...
                        pictureBox1.Image = img;
                        labelImageNum.Text = imageName.Split('_')[3].Substring(0, 3);
                        ...
                        labelImageCount.Text = (imageNum + 1).ToString() + " / " + imagePathList.Count().ToString();
        ...
        private void DrawImageToBitmap(string savePathOrg)
        {
            if (labelImageCount.Text != "XX")
            {
                bp = new Bitmap(pictureBox1.Image, new Size(Convert.ToInt16(dpiX), Convert.ToInt16(dpiY)));
        ...
        public void MainThread()
        {
            ...
            measureID = frmMain.waitMeasureDict[frmMain.glassIDNow];
            //清除舊資料
        ...
        private void SetAIDataInPoints(FilterAIData filterAIData, ImageType type)
        {
            if (type == ImageType.Corner)
            {
                ...
                dataGridViewMeasureData.Rows[0].Cells[0].Value = "1";
                dataGridViewMeasureData.Rows[0].Cells[1].Value = mapH.mapX1.ToString();
                dataGridViewMeasureData.Rows[0].Cells[2].Value = mapH.mapY1.ToString();
                points.Add(new Point(Convert.ToInt16(mapH.mapX1), Convert.ToInt16(mapH.mapY1)));

                dataGridViewMeasureData.Rows[1].Cells[0].Value = "2";
                dataGridViewMeasureData.Rows[1].Cells[1].Value = mapH.mapX2.ToString();
                dataGridViewMeasureData.Rows[1].Cells[2].Value = mapH.mapY2.ToString();
                points.Add(new Point(Convert.ToInt16(mapH.mapX2), Convert.ToInt16(mapH.mapY2)));

                dataGridViewMeasureData.Rows[2].Cells[0].Value = "3";
                dataGridViewMeasureData.Rows[2].Cells[1].Value = mapV.mapX1.ToString();
                dataGridViewMeasureData.Rows[2].Cells[2].Value = mapV.mapY1.ToString();
                points.Add(new Point(Convert.ToInt16(mapV.mapX1), Convert.ToInt16(mapH.mapY1)));

                dataGridViewMeasureData.Rows[3].Cells[0].Value = "4";
                dataGridViewMeasureData.Rows[3].Cells[1].Value = mapV.mapX2.ToString();
                dataGridViewMeasureData.Rows[3].Cells[2].Value = mapV.mapY2.ToString();
...

            {
                ...
                dataGridViewMeasureData.Rows[0].Cells[0].Value = "1";
                dataGridViewMeasureData.Rows[0].Cells[1].Value = map.mapX1.ToString();
                dataGridViewMeasureData.Rows[0].Cells[2].Value = map.mapY1.ToString();
                points.Add(new Point(Convert.ToInt16(map.mapX1), Convert.ToInt16(map.mapY1)));

                dataGridViewMeasureData.Rows[1].Cells[0].Value = "2";
                dataGridViewMeasureData.Rows[1].Cells[1].Value = map.mapX2.ToString();
                dataGridViewMeasureData.Rows[1].Cells[2].Value = map.mapY2.ToString();
        ...
        {
            string xlsxPath = Path.Combine(Application.StartupPath, "AIMeasureRecord", "AI_Data.xlsx");
            ...
            FileInfo fileInfo;
            ExcelPackage ep;
            if (!Directory.Exists(Path.Combine(Application.StartupPath, "AIMeasureRecord")))
                Directory.CreateDirectory(Path.Combine(Application.StartupPath, "AIMeasureRecord"));
